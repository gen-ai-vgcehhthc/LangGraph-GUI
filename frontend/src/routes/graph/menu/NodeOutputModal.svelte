...
<script lang="ts">
	...
	let entries = $derived(parseResultEntries(rawText));
	...
	>
		...
		<div class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<button class="rounded p-1.5 text-gray-400 hover:bg-gray-100" onclick={close}>✕</button>
			</div>
		</div>
				...
				{#each entries as entry, i (i)}
					{#if entry.kind === 'json'}
						<div class="mb-3 overflow-hidden rounded-lg border border-slate-200 shadow-sm">
							<div class="border-b border-slate-200 bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">JSON output</div>
							<div class="divide-y divide-slate-100">
								{#each Object.entries(entry.data) as [key, val]}
									<div class="flex text-sm">
										<div class="w-32 shrink-0 border-r border-slate-100 bg-slate-50 px-3 py-2 font-mono text-xs font-semibold text-slate-600 break-all">
											{key}
										</div>
										<div class="min-w-0 flex-1 px-3 py-2 text-gray-800">
											{#if typeof val === 'string'}
												<span class="whitespace-pre-wrap break-words">{unescapeText(val)}</span>
											{:else if Array.isArray(val)}
												<ul class="space-y-1">
													{#each val as item, j (j)}
					...
					{:else}
						<p class="mb-1 whitespace-pre-wrap break-words text-sm text-gray-600">{unescapeText(entry.content)}</p>
					{/if}
				{/each}
			{/if}
		</div>
