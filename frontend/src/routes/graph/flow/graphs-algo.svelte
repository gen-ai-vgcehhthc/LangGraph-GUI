...
<script lang="ts" module>
	...
	/**
	 * Open (or create) the crew designer subgraph for a CREWAI node.
	 * The subgraph is named __crew__<nodeId>.
	 */
	export function openCrewDesigner(nodeId: string) {
		const subgraphName = `__crew__${nodeId}`;
		graphs.update((g) => {
			if (!g[subgraphName]) g[subgraphName] = [];
			return g;
		});
		usingSubgraph.set(subgraphName);
	}
</script>
